...
**Constitution alignment**: Include work for (a) keeping `pnpm build/lint/type-check` green, (b) updating journey docs in `domains/{domain}/journeys/{journey}`, (c) wiring Storybook + `puck.config.tsx` when UI changes occur, and (d) refreshing `/api/health` or dashboard metrics when APIs shift.
